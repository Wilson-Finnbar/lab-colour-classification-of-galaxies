...
To create a colour-colour magnitude diagram the g magnitude needs to be subtracted from u (u-g) and plotted against the g magnitude minus the r magnitude (g-r). Where (u-g) is on the x-axis and (g-r) is on the y axis. The data from Table \ref{tab:ccmanual} is shown in the colour-colour diagram in Figure \ref{fig:parta}.
